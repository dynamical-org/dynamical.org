...
function plainText(htmlFragment) {
  // Loop until the replace reaches a fixed point so nested constructs
  // like `<scr<script>ipt>` don't reconstitute a `<script>` after a
...
@media (max-width: 1180px) {
  ...
  .md-toc .toc-h2 > ul { display: block; }
}
`;

// Scroll-spy. On every scroll, finds the last heading inside
// `.md-toc-content` whose top has crossed the top-30% "active line" and
// marks its TOC link active. If the active heading is an H3, its
// containing H2's TOC <li> also gets `expanded` so the children list
// stays visible.
//
// Computes from live bounding rects rather than tracking
// IntersectionObserver transitions: jump scrolls (anchor-link clicks,
