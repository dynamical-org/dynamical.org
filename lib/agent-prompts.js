...
const OPEN_HINT =
  "Open a dataset with dynamical_catalog.open(\"<dataset-id>\") from the dynamical-catalog package (Python 3.12+), or with Icechunk and xarray from the collection's icechunk-https asset. Resolve storage URLs from STAC at run time; never hard-code them.";
...
const PROMPTS = [
  {
    ...
    text: `Use the free, open weather and climate data published by dynamical.org. The STAC catalog at ${STAC_CATALOG} is the source of truth: read it first, then the collection.json of any dataset you use. Each collection lists the variables with units, the dimensions, the spatial and temporal extent, the license, and the icechunk-https asset to open. No credentials are needed. ${OPEN_HINT}`,
  },
  {
    id: "standard",
...
${OPEN_HINT} Forecast datasets are indexed by init_time (the model run, UTC) and lead_time; analysis datasets by time. Choose the dataset by domain, resolution, and forecast horizon, and tell me which one you chose and why.`,
  },
  {
    ...
    text: `I am making a presentation and want a chart of the 2 m temperature forecast for <city> for the next 7 days, from dynamical.org's free open weather data, with the source credited. Read ${LLMS_TXT} first. The simplest route is the point API documented at ${API_DOCS}: POST https://api.dynamical.org/v1/forecasts with a dataProductId of noaa-gfs-forecast, the city's latitude and longitude, and variables ["temperature_2m"]. It returns JSON for the latest model run with no credentials. Convert the times to local time, make a clean chart image I can drop into a slide, and caption it "NOAA GFS forecast via dynamical.org, run <initTime>".`,
  },
  {
    id: "verification",
    title: "How good was the forecast?",
    audience: "Analysts. Pairs a forecast dataset with its analysis to score it.",
    text: `Using dynamical.org's open data, evaluate how well the NOAA GFS forecast did for 2 m temperature at <latitude, longitude> over the last 30 days. Read ${LLMS_TXT} and ${STAC_CATALOG} first. Use noaa-gfs-forecast for the forecasts (indexed by init_time and lead_time) and noaa-gfs-analysis as the reference (indexed by time; it is the model's own best estimate of what happened, not independent observations, so say so in the write-up). ${OPEN_HINT} Select the single grid point before loading anything, align each forecast's valid time (init_time + lead_time) with the analysis, and report bias and RMSE by lead time with a plot.`,
  },
  {
    id: "ensemble",
    title: "A multi-model ensemble",
    audience: "Practitioners. Combines several models into one forecast with spread.",
    text: `Build a multi-model ensemble forecast of 10 m wind speed for <location> for the next 5 days from dynamical.org's open datasets. Read ${LLMS_TXT} and ${STAC_CATALOG} first, then choose from noaa-gfs-forecast, ecmwf-aifs-single-forecast, ecmwf-ifs-ens-forecast-15-day-0-25-degree, noaa-gefs-forecast-35-day, and dwd-icon-eu-forecast-5-day by each collection's domain, variables, and lead times. ${OPEN_HINT} Use the latest init_time in each dataset, select the point before loading, compute wind speed from the u and v components, align on valid time (init_time + lead_time) with the coarsest common step, weight the models equally, and show the ensemble mean and spread with each model's members.`,
  },
];
...
const SETUP = [
  {
    ...
    text: `Add dynamical.org's MCP server so you can search its open weather-data catalog directly: URL ${MCP_SERVER}, streamable HTTP transport, no authentication. Its tools are search_catalog, get_dataset_info, get_access_pattern, and list_recent_runs. If you cannot add MCP servers, read ${LLMS_TXT} and ${STAC_CATALOG} instead. Then ask me what I want to build.`,
  },
];
...
- The first option needs dynamical-catalog>=0.8.0.
...
module.exports = { PROMPTS, SETUP, MIGRATION, datasetPrompt, wordCount, STAC_CATALOG, LLMS_TXT };
