...
- We analyzed whether delays of early lead groups can detect the full run being delayed, or if they often recover
- We loaded NOAA and ECMWF dissemination advisories (see next section) to test if we could predict them through observed file arrivals.
...
These are useful as two separate signals: our arrival monitoring tells us what is happening in the files, while an agency advisory can tell us why. Advisories are available on the [pipeline status page](https://dynamical.org/status/pipeline/), in the machine readable [status feed](https://assets.dynamical.org/wxopticon/feed.json), and as a webhook event.
