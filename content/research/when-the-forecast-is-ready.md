...
{% figure "/assets/notes/threshold-candidates.png", "A scatter plot of 1,653 HRRR-on-AWS runs, each point one run's completion latency against its date. The runs form a dense band between about 105 and 120 minutes. Three horizontal candidate lines cross the plot: a dash-dotted line at 125 minutes just above the band, a dashed line at 127, and a dotted line at 146 far above everything. A density curve along the right edge shows the packed band." %}All three candidates on a single feed, and they disagree: 125, 127, and 146 minutes, flagging 10, 8, and 5 runs out of 1,653. HRRR's median and 95th percentile are only 9 minutes apart, so the raw dispersion buffer lands right on the shoulder of the band, where routine jitter crosses it. The flat 30 minutes goes the other way — on a feed that finishes in 1h47m that's a 28% cushion, while on GEFS's 26-hour run the identical 30 minutes is 2%. And ten percent of the median splits the difference for a reason that has nothing to do with this feed's actual spread.{% endfigure %}
