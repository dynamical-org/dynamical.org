...
test("mixed lead history keeps delayed, missing, unjudged, and run verdicts distinct", async ({ page }) => {
  ...
  const runLane = figure.locator('[data-lane="run"]');
  const failedInit = await failed.getAttribute("data-init-time");
  const failedRunSlot = runLane.locator(`[data-slot-init="${failedInit}"]`);
  const failedRun = failedRunSlot.locator('circle[data-status="failed"]');
  await expect(failedRun).toHaveCount(1);
  expect(await failedRun.evaluate((element) => getComputedStyle(element).fill)).toBe("rgb(197, 34, 31)");
  expect(await failedRun.getAttribute("cy")).toBe(await runLane.locator('[data-axis="x"]').getAttribute("y1"));
  await expect(failedRunSlot).toHaveAttribute("aria-label", /run.*no completion time recorded.*failed/);
  await failedRunSlot.focus();
  await failedRunSlot.press("Enter");
  await expect(figure.locator('[role="status"]')).toContainText("no completion time recorded");
  await expect(figure.locator('[role="status"]')).toContainText("failed");
  await expect(figure.locator(".sr-only li").last()).toContainText(/no completion time recorded, failed, not judged/);
